...
**Date**: 2026-08-01 · **Tool**: `cargo-llvm-cov` 0.8.7 + `llvm-tools-preview` · **Profile**: test
...
| `pool.rs` (contract core) | **95.45%** | **100.00%** | 92.63% | ✅ |
| **TOTAL** | **96.36%** | 84.92% | 97.67% | ✅ |

Test suite: **74/74 passing** (unit + proptest), plus live testnet suite **15/15 suites, 49/49 checks** (A3).

## Method

1. `cargo llvm-cov --summary-only` after full `--lib` test run (74 tests, incl. `proptest.rs` fuzz).
2. All counter instrumentation defaults; no coverage exclusions applied (`--ignore-filename-regex` unused).
3. Proptest runs are non-deterministic in case count; run executed 3,000+ op-sequence cases.

## Coverage progression (B1.1)
