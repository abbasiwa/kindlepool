...
| KI-014 | ⚠️ Accepted | — | Arbitrator weight fixed at 1 (not stake-derived) | MVP design | Documented; revisit for mainnet with verified-funding weighting | — |
| KI-015 | ⚠️ Accepted | — | Dispute fee returned to winner uses `try_transfer` (skips on failure) | Robustness choice | Documented; contract retains fee if recipient can't receive | — |
| KI-016 | ⚠️ Accepted | — | Refund loop uses `try_transfer` and skips failed recipients (funds stay in contract) | Robustness choice | Documented; funds claimable only via re-finalize — noted for mainnet (add `claim_refund`) | — |
...
|---|---|---|---|---|---|
| KI-101 | High | Jul 30 | API keys stored in-memory only (lost on restart) | Documented WARNING; DB-backed store is a Phase-2 follow-up | ⚠️ pending |
...
| KI-104 | Low | Jul 30 | Webhook dispatch fire-and-forget (no retry beyond fetch catch) | Documented; retry queue is follow-up | ⚠️ pending |
