...
export function CreatorVerification({ open, onClose }: CreatorVerificationProps) {
  const { profile, isVerified, verificationTier, startVerification, verifyEmail, verifySocial } = useCreator()
  ...
  const steps = ['Profile', 'Email', 'Social', 'Done']
        ...
        <div className="space-y-4 text-center py-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {profile.socialLinks.map((l) => (
              <span key={l.platform} className="px-3 py-1 bg-success/20 text-success text-xs rounded-full">
                {l.platform} ✓
              </span>
            ))}
        ...
        <div className="space-y-6">
            ...
            {steps.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                  step > i ? 'bg-success text-cream-50' : step === i ? 'bg-warm-300 text-cream-50' : 'bg-cream-200 text-muted-100'
                }`}>
                  {step > i ? <Check size={14} /> : i + 1}
                </div>
                {i < steps.length - 1 && <div className={`w-6 h-0.5 ${step > i ? 'bg-success' : 'bg-cream-300'}`} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <Button className="w-full" onClick={() => setStep(2)} disabled={!displayName}>Continue</Button>
            </motion.div>
          )}
          ...
          {step === 4 && (
            <motion.div key="s3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <Button variant="secondary" className="w-full" onClick={handleFinish}>
                {verificationTier === 'social' ? 'Complete' : 'Skip for now'}
              </Button>
            </motion.div>
          )}
        </div>
